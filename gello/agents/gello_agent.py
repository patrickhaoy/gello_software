...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    ...
    # ),
    # Left UR
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTA2U1PZ-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6),
        joint_offsets=(
            0*np.pi/2, -1*np.pi/2, 3*np.pi/2, -5*np.pi/2, 4*np.pi/2, -1*np.pi/2
        ),
        joint_signs=(1, -1, 1, -1, 1, 1),
        gripper_config=(7, 20, -22),
    ),
    # Right UR
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTA7NLYU-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6),
        joint_offsets=(
           -4*np.pi/2, 3*np.pi/2, 1*np.pi/2, 3*np.pi/2, 6*np.pi/2, 0*np.pi/2
        ),
        joint_signs=(1, -1, 1, -1, 1, 1),
